...
struct has_capacity {
    static constexpr bool value = E == boost::spans::dynamic_extent || E == N;
};

template<class T, std::size_t E, class U, std::size_t N>
struct compatible {
    static constexpr bool value = has_capacity<E, N>::value &&
        convertible<U, T>::value;
};
...
struct has_data<R, T, typename std::enable_if<convertible<typename
    data<R>::type, T>::value>::type> {
    static constexpr bool value = true;
};
...
struct is_range {
    static constexpr bool value = (std::is_const<T>::value ||
        ...
        !is_span<typename uncvref<R>::type>::value &&
        !is_array<typename uncvref<R>::type>::value &&
        !std::is_array<typename uncvref<R>::type>::value &&
        has_data<R, T>::value &&
        has_size<R>::value;
};
...
template<std::size_t E, std::size_t N>
struct implicit {
    static constexpr bool value = E == boost::spans::dynamic_extent ||
        N != boost::spans::dynamic_extent;
};

template<class T, std::size_t E, class U, std::size_t N>
struct copyable {
    static constexpr bool value = (N == boost::spans::dynamic_extent ||
        has_capacity<E, N>::value) && convertible<U, T>::value;
};

template<std::size_t E, std::size_t O>
struct sub {
    static constexpr std::size_t value = E == boost::spans::dynamic_extent ?
        boost::spans::dynamic_extent : E - O;
};

template<class T, std::size_t E>
struct store {
    constexpr store(T* p_, std::size_t) noexcept
        : p(p_) { }
...
struct store<T, boost::spans::dynamic_extent> {
    constexpr store(T* p_, std::size_t n_) noexcept
        : p(p_)
        , n(n_) { }
    T* p;
    std::size_t n;
...
template<class T>
struct bytes<T, boost::spans::dynamic_extent> {
    static constexpr std::size_t value = boost::spans::dynamic_extent;
};
...
public:
    ...
    template<class I,
        typename std::enable_if<E == dynamic_extent &&
            detail::convertible<I, T>::value, int>::type = 0>
    ...
    template<std::size_t N,
        typename std::enable_if<detail::has_capacity<E, N>::value,
            int>::type = 0>
    constexpr span(typename std::enable_if<true, T>::type (&a)[N]) noexcept
        : s_(a, N) { }

    template<class U, std::size_t N,
        typename std::enable_if<detail::compatible<T, E, U, N>::value,
            int>::type = 0>
    constexpr span(std::array<U, N>& a) noexcept
        : s_(a.data(), N) { }

    template<class U, std::size_t N,
        typename std::enable_if<detail::compatible<T, E, const U, N>::value,
            int>::type = 0>
    ...
    template<class R,
        typename std::enable_if<E == dynamic_extent &&
            detail::is_range<R, T>::value, int>::type = 0>
    ...
    template<class U, std::size_t N,
        typename std::enable_if<detail::implicit<E, N>::value &&
            detail::copyable<T, E, U, N>::value, int>::type = 0>
    ...
    constexpr typename std::enable_if<C == dynamic_extent,
        span<T, detail::sub<E, O>::value> >::type subspan() const {
...
template<class R>
span(R&&) -> span<typename detail::data<R>::type>;
...
template<class T, std::size_t E>
inline span<const std::byte, detail::bytes<T, E>::value>
as_bytes(span<T, E> s) noexcept
{
    return span<const std::byte,
        detail::bytes<T, E>::value>(reinterpret_cast<const
            std::byte*>(s.data()), s.size_bytes());
}

template<class T, std::size_t E>
inline typename std::enable_if<!std::is_const<T>::value,
    span<std::byte, detail::bytes<T, E>::value> >::type
as_writable_bytes(span<T, E> s) noexcept
{
    return span<std::byte,
        detail::bytes<T, E>::value>(reinterpret_cast<std::byte*>(s.data()),
            s.size_bytes());
}
#endif
